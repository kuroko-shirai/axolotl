...
__Axolotl__ — это библиотека на Go, которая обеспечивает интеллектуальную маршрутизацию команд чтения между мастер- и реплика-нодами Redis в автономной конфигурации. На основе мониторинга текущей загрузки CPU библиотека динамически выбирает: выполнять чтение с мастеров или с реплик - оптимизируя производительность и защищая ёмкость записи.
...
## Поддерживаемые стратегии выполнения

| Стратегия         | Сценарий использования     | Пример                           |
|-------------------|----------------------------|----------------------------------|
| `SingleCmd`       | Одиночная команда чтения   | `GET`, `HGET`, `EXISTS`          |
| `MultiCmd`        | Пакет команд чтения        | `DoMulti(HGET, SMEMBERS, ...)`   |
| `CacheCmd`        | Кэшированное чтение        | `DoCache(GET ...)`               |
| `MultiCacheCmd`   | Пакет кэшированных чтений  | `DoMultiCache(...)`              |

Все стратегии автоматически проверяют, что команды **только для чтения**.
